...
export default function PrintersPage() {
  ...
  async function load() {
    try {
      const [devs, st, sp, fil] = await Promise.all([
      ...
      // fetch firmware for each printer
  ...
  const activeFilament = activeSpool ? filaments.find((f) => f.id === activeSpool.filament_id) : null;
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {printers.length === 0 ? (
        <EmptyState icon={Printer} title="No printers found" description="Connect a Bambu Lab account to see your printers." />
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {printers.map((dev) => (
            <Card key={dev.dev_id} onClick={() => setSelectedPrinter(dev)} hoverable className="space-y-4">
              {/* Header */}
              ...
              {status && (
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-lg bg-zinc-900/60 border border-zinc-800/40 p-3">
                    ...
                    <p className="text-base font-bold tabular-nums">{status?.nozzle_temper ?? "—"}°C</p>
                    {status?.nozzle_target_temper && (
                      <p className="text-[10px] text-muted">Target: {status.nozzle_target_temper}°C</p>
                    )}
                  </div>
                  <div className="rounded-lg bg-zinc-900/60 border border-zinc-800/40 p-3">
                    <div className="flex items-center gap-1.5 text-[10px] text-muted mb-1">
                    ...
                    <p className="text-base font-bold tabular-nums">{status?.bed_temper ?? "—"}°C</p>
                    {status?.bed_target_temper && (
                      <p className="text-[10px] text-muted">Target: {status.bed_target_temper}°C</p>
                    )}
                  </div>
                </div>
              )}
              ...
              {printing && (
                <div
                  className="rounded-xl border border-blue-800/40 bg-blue-950/20 p-3 cursor-pointer"
                  onClick={(e) => { e.stopPropagation(); }}
                >
                  <p className="text-xs text-blue-400 font-semibold mb-2">Currently Printing</p>
                  <p className="text-sm font-medium truncate">{status?.subtask_name ?? "Unknown"}</p>
              ...
              {activeSpool && activeFilament && (
                <div
                  className="rounded-xl border border-zinc-800/40 bg-zinc-900/40 p-3 cursor-pointer hover:border-zinc-700/50 transition-colors"
                  onClick={(e) => { e.stopPropagation(); setSelectedSpool(activeSpool); }}
                >
                  <p className="text-xs text-muted font-semibold mb-2">Loaded Spool</p>
                  <div className="flex items-center gap-3">
                    <SpoolIndicator remaining={activeSpool.remaining_g} total={activeSpool.total_weight_g} color={activeFilament.color_hex} size={56} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{activeFilament.color_name}</p>
                      <p className="text-xs text-muted">{activeFilament.material}</p>
                      <div className="mt-1.5 h-1 bg-zinc-800 rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: stockColor(pct, activeFilament.color_hex) }} />
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {selectedSpool && (
        <SpoolModal
          ...
          onActivate={async () => { await api.activateSpool(selectedSpool.id); load(); }}
        />
      )}
    </div>
  );
